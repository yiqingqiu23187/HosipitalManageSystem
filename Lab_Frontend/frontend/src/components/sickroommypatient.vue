      ...
      <el-tab-pane label="患者列表">
        <div  class="text item">
          <el-table :data="patients" style="width: 100%">
              <el-table-column label="患者名字" width="150">
            ...
            <el-table-column prop="patientsex" label="性别(0:男性 1：女性)"    width="150"  column-key="patientsex"
            ...
            <el-table-column prop="type"  label="病情等级(0：轻症 1:重症 2：危重症)" width="150" column-key="type"
            ...
            <el-table-column   prop="area"  label="住院区(3：隔离区)" width="150" column-key="area"
            ...
            <el-table-column  prop="lifestate"   label="生命状态(0:康复 1：治疗中 2：死亡 3：隔离区等待" width="150" column-key="lifestate"
            ...
            <el-table-column  label="床号" width="150">
              <template slot-scope="scope">
                <span style="margin-left: 10px">{{beds[scope.$index].bednum}}</span>
              </template>
            </el-table-column>
            <el-table-column  label="房间号" width="150">
              <template slot-scope="scope">
                <span style="margin-left: 10px">{{beds[scope.$index].roomnum}}</span>
              </template>
            </el-table-column>
...
<script>
  export default {
    ...
    created:
      function () {
        this.$axios.post('/sickroommypatient', {
    ...
    methods: {
      choose:function () {
        this.$store.commit('patientid', this.patientid);
        this.$router.replace({path:'/sickroomrecord'})
      },
